...
int collide(Arbiter *arb) {
    // Setup
    Vector2 hA = v2(0.5f * arb->en_1->size.x, 0);
    Vector2 hB = v2(0.5f * arb->en_2->size.x, 0);

    Vector2 posA = arb->en_1->pos;
    Vector2 posB = arb->en_2->pos;

    Matrix4 matA = m4_scalar(1.0);
    matA = m4_translate(matA, v3(arb->en_1->pos.x, arb->en_1->pos.y, 0));
    Matrix4 matB = m4_scalar(1.0);
    matB = m4_translate(matB, v3(arb->en_1->pos.x, arb->en_1->pos.y, 0));
...
typedef struct World {
    ...
    Arbiter arbiters[MAX_ENTITY_COUNT * 2];
    UXState ux_state;
...
Arbiter *arbiter_create(Entity *en_1, Entity *en_2) {
    Arbiter *arbiter_found = 0;
    for (int i = 0; i < MAX_ENTITY_COUNT * 2; i++) {
        Arbiter *existing_arbiter = &world->arbiters[i];
        if (!existing_arbiter->is_valid) {
            arbiter_found = existing_arbiter;
            break;
        }
    }
    assert(arbiter_found, "No more free arbiters!");
    arbiter_found->is_valid = true;
    arbiter_found->en_1 = en_1;
    arbiter_found->en_2 = en_2;

    arbiter_found->numContacts = collide(arbiter_found);

    return arbiter_found;
}
                    ...
                    case ARCH_player:
                        ...
                        for (int j = 0; j < MAX_ENTITY_COUNT; j++) {
                            if (i != j) {
                                ...
                                if (en->mass == 0 && other_en->mass == 0)
                                    continue;
                                solid_entity_collision(en, other_en);

                                Arbiter newArb(en, other_en);
                            }
                        }
